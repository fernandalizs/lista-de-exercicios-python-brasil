...
def decompor_numero(numero: int):
    ...
    if numero > 999:
        ...
        if partes_numericas == 0:
            print("'Número não possui centenas, dezenas ou unidades'")
        elif partes_numericas == 1:
            print(f"'{n} = {(centenas_str + dezenas_str + unidades_str)}'")
        elif partes_numericas == 2:
            if centenas_str != '':
                segunda_parte = dezenas_str + unidades_str
                print(f"'{n} = {centenas_str} e {segunda_parte}'")
            else:
                print(f"'{n} = {dezenas_str} e {unidades_str}'")
        elif partes_numericas == 3:
            print(f"'{n} = {centenas_str}, {dezenas_str} e {unidades_str}'")
